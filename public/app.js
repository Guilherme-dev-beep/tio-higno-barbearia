...
async function loadSlots(){const slots=$('#slots'); if(!slots)return; slots.innerHTML='<small style="color:var(--muted);grid-column:1/-1">Carregando...</small>'; try{const r=await api(`/api/availability?date=${state.date}&barberId=${state.barber.id}&serviceId=${state.service.id}`); slots.innerHTML=r.slots.length?r.slots.map(t=>`<button class="slot ${state.time===t?'active':''}" data-time="${t}">${t}</button>`).join(''):'<small style="color:var(--muted);grid-column:1/-1">Sem horários disponíveis nesta data.</small>'; slots.querySelectorAll('[data-time]').forEach(el=>el.onclick=()=>{state.time=el.dataset.time; slots.querySelectorAll('[data-time]').forEach(x=>x.classList.toggle('active', x===el)); validateNext();});}catch(e){toast(e.message)} validateNext()}
